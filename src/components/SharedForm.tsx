...
export default function SharedForm({ formId, buttonText = "Submit", buttonIcon, onSuccess }: SharedFormProps) {
  ...
  const getInputBorderClass = (isValid: boolean, isTouched: boolean) => {
    if (!isTouched) return "border-white/10 focus:border-violet-400 focus:bg-white/10";
    return isValid 
      ? "border-emerald-500/60 bg-emerald-500/5 focus:border-emerald-400 focus:bg-white/10" 
      : "border-rose-500/60 bg-rose-500/5 focus:border-rose-400 focus:bg-white/10";
  };
  ...
  const labelClass = "absolute left-4 top-3.5 text-stone-400 text-sm transition-all peer-focus:-top-2.5 peer-focus:left-3 peer-focus:text-[10px] peer-focus:px-2 peer-focus:bg-[#07050f]/90 peer-focus:backdrop-blur-md peer-focus:rounded peer-focus:text-violet-400 peer-not-placeholder-shown:-top-2.5 peer-not-placeholder-shown:left-3 peer-not-placeholder-shown:text-[10px] peer-not-placeholder-shown:px-2 peer-not-placeholder-shown:bg-[#07050f]/90 peer-not-placeholder-shown:backdrop-blur-md peer-not-placeholder-shown:rounded peer-not-placeholder-shown:text-violet-400 pointer-events-none z-20";
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 w-full" noValidate>
      ...
      <div className="relative group/input">
        ...
        {(touched.name || formState.name.length > 0) && !isValidName(formState.name) && (
          <p className="text-[11px] text-rose-400 mt-1 flex items-center gap-1 font-medium pl-1">
      ...
      <div className="flex flex-col gap-1">
        <div className="flex gap-2">
          <div className="relative shrink-0 w-32">
            <select
              ...
              className="w-full h-full bg-white/5 backdrop-blur-md border border-white/10 rounded-xl pl-2.5 pr-7 py-3.5 text-white text-xs font-medium focus:outline-none focus:border-violet-400 transition-all appearance-none cursor-pointer"
            >
              {COUNTRY_CODES.map((c) => (
                <option key={c.code + c.country} value={c.code} className="bg-[#07050f] text-white">
                  {c.label}
                </option>
              ))}
            </select>
        ...
        {(touched.phone || formState.phone.length > 0) && !isValidPhone(formState.phone, countryCode) && (
          <p className="text-[11px] text-rose-400 mt-0.5 flex items-center gap-1 font-medium pl-1">
      ...
      <div className="relative mt-2 group">
        <div className="absolute -inset-1 bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 rounded-xl blur opacity-0 group-hover:opacity-50 transition duration-1000 group-hover:duration-300 pointer-events-none"></div>
        <button 
          disabled={submitStatus !== 'idle'} 
          className={`relative w-full rounded-xl py-4 font-bold tracking-widest uppercase text-sm transition-all duration-500 flex items-center justify-center gap-2 border overflow-hidden ${submitStatus !== 'idle' ? 'text-white border-transparent' : 'bg-[#07050f] border-violet-500/40 text-violet-50 hover:border-violet-400 hover:shadow-[0_0_40px_rgba(139,92,246,0.35)] active:scale-[0.98]'}`}
        >
          {submitStatus === 'idle' && <div className="absolute inset-0 -translate-x-[150%] bg-gradient-to-r from-transparent via-violet-200/15 to-transparent group-hover:animate-shimmer pointer-events-none"></div>}
          
          {submitStatus === 'loading' && <div className="absolute inset-0 bg-violet-600 rounded-xl"></div>}
          {submitStatus === 'success' && <div className="absolute inset-0 bg-emerald-600 rounded-xl"></div>}
          {submitStatus === 'error' && <div className="absolute inset-0 bg-rose-600 rounded-xl"></div>}
